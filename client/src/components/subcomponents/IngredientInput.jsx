	...
	return (
		<>
			<div className='ingredientRow'>
				<label className='ingredientLabel' htmlFor={`ingredient${index}`}>
					Zutat {index + 1}
				</label>
				<input
				...
				/>
				{listLength > 1 && (
					<button className='plusMinusButton' onClick={(e) => handleRemoveClick(e, index)}>
						-
					</button>
				)}
				{listLength - 1 === index && (
					<button className='plusMinusButton' onClick={() => handleIngredientAdd()}>
						+
					</button>
				)}
			</div>
