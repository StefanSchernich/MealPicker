...
export default function AddEditRecipe({ task }) {
  ...
  return (
    <>
      ...
      <form id="recipeForm" onSubmit={handleSubmit} encType="multipart/form-data" onKeyDown={(e) => {       // Enter soll neue Input-Zeile erzeugen, nicht Formular submitten
        if (e.key === "Enter") {
          e.preventDefault()
          return false
        }
        ...
        <div className="filterSection" style={ previewVisible ? { "display": "inline-block" } : { "display": "none" }} >
          <label className="filterHeading" htmlFor="mealImagePreview">Preview</label>
        ...
        <RadioFilterSection sectionName="difficulty" state={difficulty} dataArr={difficultyOptions} changeHandler={handleDifficultyChange} isRequired={true}>
            Schwierigkeit
        </RadioFilterSection>
        {ingredientItems}
